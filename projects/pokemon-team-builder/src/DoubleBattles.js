...
import { removeSlot1, removeSlot2 } from './redux';

class DoubleBattles extends Component {
    constructor() {
        super();
        this.state = {
            moves1WasClicked: false,
            moves2WasClicked: false,
        }
        
        this.handle1MoveClick = this.handle1MoveClick.bind(this);
        this.handle2MoveClick = this.handle2MoveClick.bind(this);
    ...
    handle1MoveClick() {
        this.setState(prevState => {
            return {
                moves1WasClicked: !prevState.moves1WasClicked
            }
        })
    }
    ...
    handle2MoveClick() {
        this.setState(prevState => {
            return {
                moves2WasClicked: !prevState.moves2WasClicked
            }
        })
    }
    ...
    mapPokemontoSlot(pokemon, slot) {
        ...
        return (
          <div className="battle">
            ...
            {slot === 1 ?
                <div onClick={this.handle1MoveClick}>
                    <h5>Moves</h5>
                    {this.state.moves1WasClicked ?
                        <ul>{moves}<h6>(Click to close)</h6></ul> :
                        <h6>(Click to expand)</h6>}
                </div>
                : 
                <div onClick={this.handle2MoveClick}>
                    <h5>Moves</h5>
                    {this.state.moves2WasClicked ?
                        <ul>{moves}<h6>(Click to close)</h6></ul> :
                        <h6>(Click to expand)</h6>}
                </div>
            }
            {slot === 1 ?
                ...
                : <button onClick={this.props.removeSlot2}>Remove</button>
            }
          </div>
        );
    }    
  ...
  render() {
      ...
      const slot2 = (Object.keys(this.props.slot2).length !== 0 && this.mapPokemontoSlot(this.props.slot2, 2));
      
    return (
      <div className="doubleBattleContainer">
...
export default connect(state => state, { removeSlot1, removeSlot2 })(DoubleBattles);
